...
* In a solution with a large amount of throughput, it’s not uncommon to selectively optimize the number of paths indexed to reduce both the latency and RU charge of individual operations that create or modify an item. 
* To accomplish this, you would need to create a container using a custom indexing policy.
* a simple example JSON document. This document has multiple properties, but the goal is to only index the categoryName and name properties.
    ``` mark
        {
    ...
    ```
* indexing policy would start by excluding all possible paths, and then opt-in to only including the /name/? and /categoryName/? paths.
    ``` mark
        {
    ...
    ```
* We are excluding all paths here for demonstration purposes. In general, its much better to include all paths by default and only exclude specific paths.
* C# code with a Microsoft.Azure.Cosmos.IndexingPolicy
    ``` mark
...
* Evaluate composite indexes
    * For example, SQL queries that filters on multiple properties simultaneously, called a <ins>composite index</ins> that combines two paths in a specific order.
    * SELECT * FROM products p WHERE p.name = "Road Saddle" AND p.price > 50
    * composite index could be: (name ASC, price ASC) or  (name DESC, price ASC)
    * Another Example: SELECT * FROM products p ORDER BY p.price ASC, p.name ASC. 
    * A composite index (name ASC, price ASC) won't work in this example. Composite indexes will work in this scenario: (price ASC, name ASC) (price DESC, name DESC)
    * To create a raw JSON indexing policy with a composite index of (name ASC, price DESC):
        ``` mark
            {
              "indexingMode": "consistent",
              "automatic": true,
...
* An Azure Cognitive Search instance is comprised of a few core components:
    1. <ins>Indexes</ins> that contain JSON documents that are searchable
    2. <ins>Indexers</ins> to crawl data from various data sources and insert them into indexes
    3. <ins>Data Sources</ins> that connect Azure Cognitive Search to various data platforms
* ![image](https://github.com/mkader/Developer-Tools/assets/3132680/25ccce5e-bf7e-4ca9-bc69-987c5c5bb702)
* In the case of Azure Cosmos DB for NoSQL (a container as a data source), create a query and a frequency that the indexer will use to crawl data, and create a target index where the resulting searchable JSON documents are stored.
* Customizing the index - Each field in the index should be configured to enable or disable features when searching. Allow extra search functionality on specific fields:
*   Feature|Description
...
* The default query for the CosmosDB data source: SELECT * FROM c WHERE c._ts >= @HighWaterMark ORDER BY c._ts
* timestamp (_ts property) >= a built-in high watermark field (comes from a built-in change detection policy that attempts to identify whether an item has been changed or not).

# Manage a data deletion detection policy
* If an item is deleted from a container in CosmosDB, that item may not be deleted from the index in Azure Cognitive Search. To enable tracking of deleted items, you must configure a policy to track when an item is deleted.
...
* Azure Functions is a service that offers serverless blocks of code that can run logic (more completx) on-demand. 
* Typically, the function code performs a small or complex task (extranal action) and returns a response. 
* Each function starts with some external event, called a <ins>trigger</ins>. 
* Diagram illustrating an Azure Function with a generic trigger, input, and output.
* ![image](https://github.com/mkader/Developer-Tools/assets/3132680/167a3c26-143d-423e-bf00-403effc0deb8)
* CosmosDB has support for all three types of bindings in Azure Functions:
*   Binding|Description
    |-|-
...
* ![image](https://github.com/mkader/Developer-Tools/assets/3132680/000e44b8-ca7e-4a1f-91fd-c56bb0a32520)
*  Example, Trigger function on changes in the change feed, includes a method parameter of type IReadOnlyList<Document> that has an enumerated list of the current batch of changes. The name in the binding is set to changes. Within the function code, your logic can use a foreach loop to iterate over all changes in the current batch.
    ``` mark
    ...
    ```
# Understand indexes
* Every CosmosDB container has a built-in policy that determines how each item should be indexed.
* for example - a product named Touring-1000 Blue with three tags (bike, touring, and blue). traversal paths image
    ``` mark
        {
          "name": "Touring-1000 Blue",
          "tags": [
    ...
    ```
* ![img](https://learn.microsoft.com/en-us/training/wwl-data-ai/define-indexes-azure-cosmos-db-sql-api/media/2-tour-tree.png)
* a product named Mountain-400-W Silver that only contains two tags (bike and silver) with sku
    ``` mark
        {
          "name": "Mountain-400-W Silver",  
          "sku": "BK-M38S-38",
    ...
    ```
* ![img](https://learn.microsoft.com/en-us/training/wwl-data-ai/define-indexes-azure-cosmos-db-sql-api/media/2-mountain-tree.png)
* To conceptualize our container’s index, the index as a union of all trees for each item in the container. Altogether, this creates an <ins>inverted index</ins>
* ![img](https://learn.microsoft.com/en-us/training/wwl-data-ai/define-indexes-azure-cosmos-db-sql-api/media/2-inverted-tree.png)
* sql search SELECT * FROM products p WHERE p.sku = 'BK-M38S-38'
* ![img](https://learn.microsoft.com/en-us/training/wwl-data-ai/define-indexes-azure-cosmos-db-sql-api/media/2-search-tree-01.png)
* another sql SELECT p.id FROM products p WHERE p.name = 'Touring-1000 Blue'
* ![img](https://learn.microsoft.com/en-us/training/wwl-data-ai/define-indexes-azure-cosmos-db-sql-api/media/2-search-tree-02.png)
* Understand indexing policies - The default indexing policy consists of the following settings
    1. The inverted index is updated for all create, update, or delete operations on an item
    2. All properties for every item is automatically indexed
...
* Review indexing policy strategies
    1. Included path: /category/name/?
    2. Excluded path: /category/*
    * The exclude path excludes all possible properties within the category path, 
    * however the include path is more precise and specifically includes the category.name property. 
    * The result is that all properties within category are not indexed, with the sole exception being the category.name property.

# Create cross-product queries
* A JOIN in CosmosDB is different from a JOIN in a relational db as its only scope is a single item. A JOIN creates a cross-product between different sections of a single item.
* Example : a name property and an array with three objects that each have their own group property:
    ``` mark
        {
            "id": "E08E4507-9666-411B-AAC4-519C00596B0A",
    ...
    ```
* If you create a cross-product of the name and group properties:
    ``` mark
        [
            {
                "name": "Men's Bib-Shorts",
...
* another example
    ...
    ```
* Implement correlated subqueries - optimize JOIN expressions further by writing subqueries to filter the number of array items
* What if we want to filter out the group tags?  rewrite our JOIN expression by writing a subquery to filter out the group tags using a subquery:
    ``` mark
...
* Implement variables in queries
    ``` mark
        ...
        QueryDefinition query = new (sql).WithParameter("@lower", 500);
    ```
* Paginate query results - The Microsoft.Azure.Cosmos.Container class supports asynchronous streams for the simplest and easiest way to iterate over multiple pages of results.
    ``` mark
...
# Understand SQL query language
    ``` mark
        ...
        SELECT
            p.name, 
            p.categoryName, //  p.categoryName AS category,
            p.price
        FROM 
            products p
        WHERE
...

        {
            ...
            "category": or "categoryName": "Components, Bottom Brackets",
            "price": 53.99
        }
...
* Implement type-checking in queries
    ``` mark
        ...
        SELECT p.id, p.price FROM products p WHERE IS_STRING(p.price)
    ```
* Use built-in functions
    ``` mark
        ...
        SELECT * FROM products p WHERE p.retirementDate >= GetCurrentDateTime()
    ```
# Measure query cost
* The QueryRequestOptions class is also helpful in measuring the cost of a query in request units per second (or RU/s).
    ``` mark
        QueryRequestOptions options = new()
        {
            MaxItemCount = 25 //the number of items returns
        };
        ...
        while(iterator.HasMoreResults)
        {
            ...
            // current RU/s charge for the result page 
            Console.WriteLine($"RUs:\t\t{response.RequestCharge:0.00}"); 
        
            totalRUs += response.RequestCharge;
        }

        // Aggregate all of the RU/s charges for each page of results, the total RU/s consumed. 
